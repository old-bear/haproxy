...
static inline int haproxy_to_comlog_level(int ha_level)
{
    switch(ha_level){
        case 0:
        case 1:
...
#define Alert(fmt, arg...) \
do { \
	com_writelog_c(1, "[" __FILE__ ":" UL_LOGLINE(__LINE__) "]" fmt, ##arg); \
} while (0)

#define Warning(fmt, arg...) \
do { \
	com_writelog_c(2, "[" __FILE__ ":" UL_LOGLINE(__LINE__) "]" fmt, ##arg); \
} while (0)

#define qfprintf(out, fmt, arg...) \
do { \
	com_writelog_c(4, "[" __FILE__ ":" UL_LOGLINE(__LINE__) "]" fmt, ##arg); \
} while (0)

#define send_log(p, level, fmt, arg...) \
do { \
	com_writelog_c(haproxy_to_comlog_level(level), "[" __FILE__ ":" UL_LOGLINE(__LINE__) "]" fmt, ##arg); \
} while (0)


#define __send_log(p, level, message, size) \
do { \
	com_writelog_c(haproxy_to_comlog_level(level), "[" __FILE__ ":" UL_LOGLINE(__LINE__) "]" "%s", message); \
} while (0)
